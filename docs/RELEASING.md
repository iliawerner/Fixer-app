...
the icon-stripped build from CI. The current release line is `0.3.x`.

## Release policy

- Release tags use semantic versions, for example `v0.3.0`.
...
release notes. The 0.3.0 release uses build 5.
...
```sh
mkdir -p .build/design-renders-0.3.0
xcodebuild test \
  -project Fixer.xcodeproj \
  -scheme Fixer \
  -destination 'platform=macOS' \
  ...
  FIXER_PREVIEW_DIR="$PWD/.build/design-renders-0.3.0" \
  CODE_SIGN_IDENTITY=-
```
...
xcodebuild build \
  ...
  -derivedDataPath .build/release-0.3.0 \
  -disableAutomaticPackageResolution \
  -onlyUsePackageVersionsFromResolvedFile \
...
The 0.3.0 public beta remains ad-hoc signed and is not notarized. For an ad-hoc
...
`.build/release-0.3.0/Build/Products/Release/fixer.app`.

```sh
ditto -c -k --sequesterRsrc --keepParent \
  .build/release-0.3.0/Build/Products/Release/fixer.app \
  .build/release-0.3.0/notary-upload.zip

xcrun notarytool submit .build/release-0.3.0/notary-upload.zip \
  --keychain-profile FIXER_NOTARY \
  --wait

xcrun stapler staple .build/release-0.3.0/Build/Products/Release/fixer.app
xcrun stapler validate .build/release-0.3.0/Build/Products/Release/fixer.app
spctl -a -vv --type exec .build/release-0.3.0/Build/Products/Release/fixer.app
```
...
./scripts/package-release.sh \
  ...
  5 \
  .build/release-0.3.0/Build/Products/Release/fixer.app \
  .build/release-0.3.0-assets
```

Expected assets for the current ad-hoc beta:

- `Fixer-0.3.0-macOS-universal-adhoc.zip`
- `SHA256SUMS.txt`
...
```sh
git tag -a v0.3.0 -m "Fixer 0.3.0 — History and Recovery"
git push origin v0.3.0

gh release create v0.3.0 \
  .build/release-0.3.0-assets/Fixer-0.3.0-macOS-universal-adhoc.zip \
  .build/release-0.3.0-assets/SHA256SUMS.txt \
  --verify-tag \
  --draft \
  --title "Fixer 0.3.0 — History and Recovery" \
